 ...
 */
public class Subject implements JsonDeserializer<Subject> {
    public static List<Subject> cache;
    public int id;
    public String name;
    ...
    public static void populateCache(URL url, String sessionId, Gson serializer, Gson deserializer) throws WebUntisConnectionException, WebUntisParseException {
        Request request = new Request("getSubjects");
        Type resultType = new TypeToken<WebUntisResult<List<Subject>>>(){}.getType();
        WebUntisResult<List<Subject>> result = Request.send(url, sessionId, serializer.toJson(request), resultType, deserializer);
        assert result != null;
        cache = result.result;
    }
}
